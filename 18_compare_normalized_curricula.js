...
async function main() {
  try {
    ...
    haridusportaalNames.forEach((name) => {
      if (haridusilmNames.has(name)) {
        exactMatches.push(name);
      } else {
        onlyInHaridusportaal.push(name);
      }
    });

    haridusilmNames.forEach((name) => {
      if (!haridusportaalNames.has(name)) {
        onlyInHaridusilm.push(name);
      }
    });
